...
class BME280(object):
    ...
    def __init__(
        ...
        osamples = [OSAMPLE_0, OSAMPLE_1, OSAMPLE_2, OSAMPLE_4, OSAMPLE_8, OSAMPLE_16]
        ...
        self.i2c.writeto_mem(self.address, BME280_REGISTER_CONTROL, bytearray([0x24]))
        time.sleep(0.002)
        self.t_fine = 0
        ...
        self.i2c.writeto_mem(self.address, BME280_REGISTER_CONTROL_IIR, self._l1_barray)
        time.sleep(0.002)
        self._l1_barray[0] = self.humidity_mode
        self.i2c.writeto_mem(self.address, BME280_REGISTER_CONTROL_HUM, self._l1_barray)

    def read_raw_data(self, result):
        self._l1_barray[0] = self.pressure_mode << 5 | self.temperature_mode << 2 | 1

        self.i2c.writeto_mem(self.address, BME280_REGISTER_CONTROL, self._l1_barray)

        osamples_1_16 = [OSAMPLE_1, OSAMPLE_2, OSAMPLE_4, OSAMPLE_8, OSAMPLE_16]
        ...
        while (
            unpack(
                "<H", self.i2c.readfrom_mem(self.address, BME280_REGISTER_STATUS, 2)
            )[0]
            & 0x08
        ):
            time.sleep(0.001)
    ...
    def altitude(self, pressure_sea_level=1013.25):
        ...
        return 44330 * (
            1 - ((float(pi + pd) / 100) / pressure_sea_level) ** (1 / 5.255)
        )
